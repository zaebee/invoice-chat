                ...
                <div className="flex-1 overflow-y-auto custom-scrollbar">
                    ...
                    {sidebarTab === 'map' && (
                        <div className="h-full bg-slate-100 flex flex-col items-center justify-center text-slate-400 relative p-6 text-center">
                            ...
                            <h4 className="text-slate-800 font-bold text-sm mb-1">Pickup Location</h4>
                            ...
                            <div className="w-full aspect-square bg-slate-200 rounded-xl border border-slate-300 flex items-center justify-center overflow-hidden relative">
                                ...
                                <span className="text-xs font-bold relative z-10 bg-white/80 px-3 py-1 rounded-full backdrop-blur-sm">Map Placeholder</span>
                            </div>
                        </div>
                    )}
                    ...
                    {sidebarTab === 'profile' && (
                        <div className="p-4 space-y-6">
                            <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden p-6 flex flex-col items-center relative">
                                ...
                                <div className="flex gap-2 w-full pt-4 border-t border-slate-100">
                                    <div className="flex-1 flex flex-col items-center p-2 bg-slate-50 rounded-lg">
                                        <span className="text-[10px] text-slate-400 uppercase font-bold">Leases</span>
                            ...
                            <div className="space-y-3">
                                <h4 className="text-xs font-bold text-slate-400 uppercase px-2">Rider Details</h4>
                                <div className="bg-white rounded-xl border border-slate-200 p-4 shadow-sm space-y-4">
                                    <InputGroup label="Full Name *" value={leaseData.renter.surname || chat.user.name} onChange={(v) => handlers.updateLease('renter', 'surname', v)} placeholder="Enter Rider Name" />
                                    <InputGroup label="Contact Info *" value={leaseData.renter.contact || ''} onChange={(v) => handlers.updateLease('renter', 'contact', v)} placeholder="Phone or Email" />
                                    <InputGroup label="Passport / ID" value={leaseData.renter.passport || ''} onChange={(v) => handlers.updateLease('renter', 'passport', v)} placeholder="Passport Number" />
                                </div>
                            </div>

                            <div className="space-y-3">
                                <h4 className="text-xs font-bold text-slate-400 uppercase px-2">Owner Details</h4>
                                <div className="bg-white rounded-xl border border-slate-200 p-4 shadow-sm space-y-4">
                                    <InputGroup label="Rent Service Name *" value={leaseData.owner.surname} onChange={(v) => handlers.updateLease('owner', 'surname', v)} helperText="Shown on contract" />
                                    <InputGroup label="Business Address" value={leaseData.owner.address} onChange={(v) => handlers.updateLease('owner', 'address', v)} placeholder="Full Address" />
                                </div>
                            </div>
                        </div>
                    )}
