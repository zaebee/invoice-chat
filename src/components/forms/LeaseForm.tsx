...
const LeaseForm: React.FC<LeaseFormProps> = ({ data, lang, handlers, compact = false }) => {
  ...
  const VehicleStep = (
      <div className="space-y-4">
        ...
        <div className="pt-4 border-t border-slate-200 dark:border-slate-700 mt-4">
             <h4 className="text-xs font-bold text-slate-500 dark:text-slate-400 uppercase mb-3 bg-slate-50 dark:bg-slate-800 p-1.5 rounded">{t('grp_vehicle', lang)}</h4>
  ...
  const ScheduleStep = (
      <div className="space-y-4">
         <div className="bg-white dark:bg-slate-900 p-3 rounded-lg border border-slate-200 dark:border-slate-700 shadow-sm">
            <span className="text-[11px] font-bold text-blue-600 dark:text-blue-400 uppercase block mb-3 tracking-wider">{t('grp_pickup', lang)}</span>
         ...
         <div className="bg-white dark:bg-slate-900 p-3 rounded-lg border border-slate-200 dark:border-slate-700 shadow-sm">
            <span className="text-[11px] font-bold text-red-600 dark:text-red-400 uppercase block mb-3 tracking-wider">{t('grp_return', lang)}</span>
             <div className="grid grid-cols-2 gap-3 mb-2">
      ...
      <div className="space-y-4">
             ...
             <div className="pt-4 border-t border-slate-200 dark:border-slate-700 mt-2">
                 <div className="flex justify-between items-center mb-3">
                    <span className="text-xs font-bold text-slate-500 dark:text-slate-400 uppercase">{t('grp_extra', lang)}</span>
                    <button onClick={addExtraOption} className="text-blue-600 dark:text-blue-400 bg-blue-50 dark:bg-blue-900/20 p-1.5 rounded hover:bg-blue-100 dark:hover:bg-blue-900/40 transition-colors"><Plus size={16}/></button>
                 </div>
                 {data.extraOptions.map((opt, i) => (
                     <div key={i} className="flex gap-1.5 mb-2 items-center">
                         <input 
                            className="flex-1 min-w-0 text-sm px-2.5 py-2 border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-900 text-slate-900 dark:text-white rounded-lg focus:ring-2 focus:ring-blue-100 dark:focus:ring-blue-900/30 outline-none placeholder:text-slate-400 dark:placeholder:text-slate-600" 
                            value={opt.name} 
                            onChange={e => updateExtraOption(i, 'name', e.target.value)} 
                            placeholder="Option Name" 
                         />
                         <input 
                            className="w-20 text-sm px-1 py-2 text-center border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-900 text-slate-900 dark:text-white rounded-lg focus:ring-2 focus:ring-blue-100 dark:focus:ring-blue-900/30 outline-none placeholder:text-slate-400 dark:placeholder:text-slate-600" 
                            type="number" 
                            value={opt.price} 
                         ...
                         <button 
                            ...
                            className="shrink-0 text-slate-400 hover:text-red-500 dark:hover:text-red-400 p-1.5 hover:bg-slate-100 dark:hover:bg-slate-800 rounded transition-colors"
                         >
                            <Trash2 size={16} />
                         </button>
                     </div>
             ...
             <div className="grid grid-cols-2 gap-3 border-t dark:border-slate-700 pt-4 mt-2 bg-slate-50 dark:bg-slate-800 p-3 rounded-lg border border-slate-100 dark:border-slate-700">
  ...
  const PartiesStep = (
      <div className="space-y-5">
             <div>
                 <span className="text-[11px] font-bold text-gray-500 dark:text-slate-400 block mb-3 uppercase bg-slate-100 dark:bg-slate-800 px-2 py-1 rounded tracking-wider">{t('grp_owner', lang)}</span>
             ...
             <div className="border-t border-slate-200 dark:border-slate-700 pt-4">
                 <span className="text-[11px] font-bold text-gray-500 dark:text-slate-400 block mb-3 uppercase bg-slate-100 dark:bg-slate-800 px-2 py-1 rounded tracking-wider">{t('grp_renter', lang)}</span>
             ...
             <div className="border-t border-slate-200 dark:border-slate-700 pt-4">
                <span className="text-[11px] font-bold text-gray-500 dark:text-slate-400 block mb-2 uppercase tracking-wider">{t('lbl_terms', lang)}</span>
                <textarea 
                    className="w-full h-32 text-[10px] p-3 border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-900 text-slate-900 dark:text-white rounded-lg font-mono focus:ring-4 focus:ring-blue-500/10 dark:focus:ring-blue-500/30 focus:border-blue-500 dark:focus:border-blue-500 outline-none leading-relaxed transition-all"
                    value={data.terms}
