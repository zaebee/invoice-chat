...
export const RightPanel: React.FC<RightPanelProps> = ({ 
    ...
    return (
        <div className={`bg-white dark:bg-slate-900 border-l border-slate-100 dark:border-slate-800 hidden xl:flex flex-col h-full shadow-lg z-20 transition-all duration-300 ease-in-out overflow-hidden ${isOpen ? 'w-[360px] opacity-100' : 'w-0 opacity-0 border-none'}`}>
            <div className="w-[360px] h-full flex flex-col bg-slate-50/50 dark:bg-slate-900/50">
                {/* Sidebar Header/Tabs */}
                <div className="bg-white dark:bg-slate-900 px-4 py-3 border-b border-slate-200 dark:border-slate-800 shrink-0">
                    <div className="flex bg-slate-100 dark:bg-slate-800 p-1 gap-1 rounded-xl">
                        <button onClick={() => setSidebarTab('details')} className={`flex-1 py-2 rounded-lg text-xs font-bold flex items-center justify-center gap-2 transition-all ${sidebarTab === 'details' ? 'bg-white dark:bg-slate-700 text-blue-600 dark:text-blue-400 shadow-sm ring-1 ring-black/5' : 'text-slate-500 dark:text-slate-400 hover:text-slate-700 dark:hover:text-slate-200'}`}>
                            <FileEdit size={14} /> {t('rp_details', lang)}
                        </button>
                        <button onClick={() => setSidebarTab('map')} className={`flex-1 py-2 rounded-lg text-xs font-bold flex items-center justify-center gap-2 transition-all ${sidebarTab === 'map' ? 'bg-white dark:bg-slate-700 text-blue-600 dark:text-blue-400 shadow-sm ring-1 ring-black/5' : 'text-slate-500 dark:text-slate-400 hover:text-slate-700 dark:hover:text-slate-200'}`}>
                            <MapPin size={14} /> {t('rp_map', lang)}
                        </button>
                        <button onClick={() => setSidebarTab('profile')} className={`flex-1 py-2 rounded-lg text-xs font-bold flex items-center justify-center gap-2 transition-all ${sidebarTab === 'profile' ? 'bg-white dark:bg-slate-700 text-blue-600 dark:text-blue-400 shadow-sm ring-1 ring-black/5' : 'text-slate-500 dark:text-slate-400 hover:text-slate-700 dark:hover:text-slate-200'}`}>
                ...
                <div className="flex-1 overflow-y-auto custom-scrollbar">
                    ...
                    {sidebarTab === 'map' && (
                        <div className="h-full bg-slate-100 dark:bg-slate-800 flex flex-col items-center justify-center text-slate-400 dark:text-slate-500 relative p-6 text-center">
                            <div className="w-16 h-16 bg-white dark:bg-slate-700 rounded-full flex items-center justify-center shadow-sm mb-4">
                                <MapPin size={32} className="text-blue-500" />
                            </div>
                            <h4 className="text-slate-800 dark:text-white font-bold text-sm mb-1">{t('rp_pickup_location', lang)}</h4>
                            <p className="text-xs mb-6 max-w-[200px]">{leaseData.owner.address}</p>
                            <div className="w-full aspect-square bg-slate-200 dark:bg-slate-700 rounded-xl border border-slate-300 dark:border-slate-600 flex items-center justify-center overflow-hidden relative">
                                <div className="absolute inset-0 bg-[url('https://upload.wikimedia.org/wikipedia/commons/e/ec/World_map_blank_without_borders.svg')] bg-cover opacity-10 dark:opacity-5 grayscale dark:invert"></div>
                                <span className="text-xs font-bold relative z-10 bg-white/80 dark:bg-slate-900/80 px-3 py-1 rounded-full backdrop-blur-sm text-slate-700 dark:text-slate-200">{t('rp_map_placeholder', lang)}</span>
                            </div>
                        </div>
                    )}
                    ...
                    {sidebarTab === 'profile' && (
                        <div className="p-4 space-y-6">
                            <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-sm overflow-hidden p-6 flex flex-col items-center relative">
                                ...
                                <div className="relative z-10 w-24 h-24 rounded-full bg-white dark:bg-slate-800 p-1 mb-3 shadow-md">
                                    ...
                                    <div className={`absolute bottom-1 right-1 w-5 h-5 rounded-full border-[3px] border-white dark:border-slate-800 ${chat.user.status === 'online' ? 'bg-green-500' : 'bg-slate-300'} hidden`}></div>
                                </div>
                                ...
                                <div className="flex gap-2 w-full pt-4 border-t border-slate-100 dark:border-slate-800">
                                    <div className="flex-1 flex flex-col items-center p-2 bg-slate-50 dark:bg-slate-800 rounded-lg">
                            ...
                            <div className="space-y-3">
                                ...
                                <div className="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-700 p-4 shadow-sm space-y-4">
